...
# a patient.
#
# The update method in the Patient class is the inner loop of the simulation. It modifies the state of the virus population 
# for a single time step and returns the total virus population at the end of the time step. At every time step of the 
# simulation, each virus particle has a fixed probability of being cleared (eliminated from the patient's body). If the virus
#  particle is not cleared, it is considered for reproduction. If you utilize the population density correctly, you shouldn't 
# need to provide an explicit check that the virus population exceeds maxPop when you are calculating how many offspring are 
# added to the population -- you just calculate the new population density and use that for the next call to update.
#
# Unlike the clearance probability, which is constant, the probability of a virus particle reproducing is a function of the 
# virus population. With a larger virus population, there are fewer resources in the patient's body to facilitate reproduction, 
...
#
# To summarize, update should first decide which virus particles are cleared and which survive by making use of the doesClear 
# method of each SimpleVirus instance, then update the collection of SimpleVirus instances accordingly. With the surviving 
# SimpleVirus instances, update should then call the reproduce method for each virus particle. Based on the population density 
# of the surviving SimpleVirus instances, reproduce should either return a new instance of SimpleVirus representing the offspring
# of the virus particle, or raise a NoChildException indicating that the virus particle does not reproduce during the current 
# time step. The update method should update the attributes of the patient appropriately under either of these conditions. After 
# iterating through all the virus particles, the update method returns the number of virus particles in the patient at the end of
# the time step.
#
# Hint: mutating objects
#
# Note that the mapping between time steps and actual time will vary depending on the type of virus being considered, but for 
# this problem set, think of a time step as a simulated hour of time.
#
...
from ProblemSet3 import NoChildException

def getChoice(P):
    """
    Returns True or False randomly depending on the probability P
    :param P: Probability
    :return True if random.randint < P * 100, else False:
    """
...
class SimpleVirus(object):
    ...
    def doesClear(self):
        ...
        False.
        """
        return getChoice(self.getClearProb())
...
class Patient(object):
    ...
    def update(self):
        ...
        for virus in temp_viruses:
            try:
                reproduce = virus.reproduce(currentPopDen)
                if reproduce:
...
def main():
    ...
    arg1 = test_data[0]['input'][0]
    arg2 = test_data[0]['input'][1]
    
    virus = SimpleVirus(arg1, arg2)
    patient = Patient([virus], 100)

    for i in range(100):
        print(i, patient.update())
