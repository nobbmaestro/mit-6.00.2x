...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        while not self.room.is_position_in_room(self.position):
            ...
            if 0 <= temp_angle < 90:
                if self.position.get_y() > self.position.get_x():
            ...
            elif 180 <= temp_angle < 270:
                if self.position.get_x() < 0 < self.position.get_y():
                    self.direction = random.randint(90, 180)
                else:
                    self.direction = random.randint(270, 360)

            # if 270 <= x < 360: sin(x) < 0 and cos(x) > 0
            elif 270 <= temp_angle < 360:
