...
from tk import *  # noqa: F403


# pylint: disable=C0103, I1101, W0401, R0902, E0602, W0621
class RobotVisualization:
    """RobotVisalization Class."""

    def __init__(self, num_robots, width, height, delay=0.2):
        """Initialize a visualization with the specified parameters."""
        ...
        self.master = Tk()  # noqa: F405
        self.w = Canvas(self.master, width=500, height=500)  # noqa: F405
        self.w.pack()
        self.master.update()
        ...
        self.robots = None
        self.text = self.w.create_text(25, 0, anchor=NW, text=self._status_string(0, 0))  # noqa: F405
        self.time = 0
        self.master.update()

    def _status_string(self, time, num_clean_tiles):
        """Return an appropriate status string to print."""
    ...
    def _map_coords(self, x, y):
        """Map grid positions to window positions (in pixels)."""
        return (250 + 450 * ((x - self.width / 2.0) / self.max_dim),
                250 + 450 * ((self.height / 2.0 - y) / self.max_dim))

    def _draw_robot(self, position, direction):
        """Return a polygon representing a robot with the specified parameters."""
    ...
    def update(self, room, robots):
        """Redraw the visualization with the specified room and robot state."""
        ...
        for robot in robots:
            ...
            self.robots.append(self._draw_robot(robot.get_robot_position(), robot.get_robot_direction()))
        # Update text
        ...
        self.text = self.w.create_text(
            ...
            0,
            anchor=NW,  # noqa: F405
            text=self._status_string(self.time, room.get_num_cleaned_tiles()))
        self.master.update()
