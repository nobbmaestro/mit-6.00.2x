...
Recall that in each trial, the objective is to determine how many time-steps are on average needed before a specified
fraction of the room has been cleaned. Implement the following function:
...
The first six parameters should be self-explanatory. For the time being, you should pass in StandardRobot for the
robot_type parameter, like so:
  avg = runSimulation(10, 1.0, 15, 20, 0.8, 30, StandardRobot)

Then, in runSimulation you should use robot_type(...) instead of StandardRobot(...) whenever you wish to instantiate
a robot. (This will allow us to easily adapt the simulation to run with different robot implementations, which you'll
encounter in Problem 6.)
...
You should also check your simulation's output for speeds other than 1.0. One way to do this is to take the above test
cases, change the speeds, and make sure the results are sensible.
...
def run_simulation(num_robots, speed, width, height, min_coverage, num_trials, robot_type):
    """Run NUM_TRIALS trials of the simulation.

    The simulation is run with NUM_ROBOTS robots of type ROBOT_TYPE, each with
    speed SPEED, in a room of dimensions WIDTH x HEIGHT.

    Args:
        num_robots (int): an int (num_robots > 0)
        speed (float): a float (speed > 0)
        width (int): an int (width > 0)
        height (int): an int (height > 0)
        min_coverage (float): a float (0 <= min_coverage <= 1.0)
        num_trials (int): an int (num_trials > 0)
        robot_type: class of robot to be instantiated (e.g. StandardRobot or RandomWalkRobot)

    Returns:
        float: the mean number of time-steps needed to clean the fraction MIN_COVERAGE of the room.
    ...
    for _ in range(num_trials):
        ...
        for clock_tick in range(1000):
            for robot in list_of_robots:
                robot.update_position_and_clean()

            if room.get_num_cleaned_tiles() / (height*width) >= min_coverage:
                sim_results.append(clock_tick)
                break

    return sum(sim_results) / num_trials + 1
