...
Another way to transport the cows is to look at every possible combination of trips and pick the best one. This is an
example of a brute force algorithm.

Implement a brute force algorithm to find the minimum number of trips needed to take all the cows across the universe
in the function brute_force_cow_transport. The function returns a list of lists, where each inner list represents a
...
and returns a generator that contains all the possible partitions of this list, from 0-partitions to n-partitions,
where n is the length of this list.

You can review more on generators in the Lecture 2 Exercise 1. To use generators, you must iterate over the generator
to retrieve the elements; you cannot index into a generator! For instance, the recommended way to call get_partitions
on a list [1,2,3] is the following. Try it out in ps1_partitions.py to see what is printed!
...
Since this trip contains 16 tons of cows, it is over the weight limit and does not work. Then the algorithm will try
fitting them on all combinations of two trips. Suppose it first tries [["Jesse", "Maggie"], ["Maybel", "Callie"]].
...
The final result is then [["Jesse", "Callie"], ["Maybel", "Maggie"]]. Note that depending on which cow it tries first,
the algorithm may find a different, optimal solution. Another optimal result could be
[["Jesse", "Maybel"],["Callie", "Maggie"]].
"""

from ProblemSet1.lib import get_partitions


def get_weight_of_trip(cows, trip):
    """Auxillary function for brute_force_cow_transport()."""
    total_weight_of_the_trip = 0
    for cow in trip:
        total_weight_of_the_trip += cows[cow]

    return total_weight_of_the_trip


def brute_force_cow_transport(cows, limit=10):
    """Return the trips by utilizing brute force algorythm.
    ...
    for partition in get_partitions(cows_copy):
        ...
        for trip in partition:
            if get_weight_of_trip(cows, trip) > limit:
                trip_leg.append(False)
            else:
                trip_leg.append(True)

        if False not in trip_leg and len(partition) <= num_trips:
