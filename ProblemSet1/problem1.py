...
# ton, the 1 ton cow will get put onto the spaceship.
#
# Implement a greedy algorithm for transporting the cows back across space in the function greedy_cow_transport. The function 
# returns a list of lists, where each inner list represents a trip and contains the names of cows taken on that trip.
#
...
# Since Maggie will not fit on this trip, the greedy algorithm picks Maybel, the heaviest cow that will still fit. Now there is 
# only 1 ton of space left, and none of the cows can fit in that space, so the first trip is [Jesse, Maybel].

# For the second trip, the greedy algorithm first picks Maggie as the heaviest remaining cow, and then picks Callie as the last 
# cow. Since they will both fit, this makes the second trip [[Maggie], [Callie]].
...
def greedy_cow_transport(cows,limit=10):
    ...
    for tripCount in range(len(cows)):
        ...
        while loopVariable:
            ...
            for i in range(len(list_of_keys)):
                if list_of_values[i] == max(list_of_values) and list_of_values[i] + totalWeightOfTheTrip <= limit:
                    key = i
                    break
                elif list_of_values[i] + totalWeightOfTheTrip <= limit:
                    if list_of_values[i] < list_of_values[key]:
                        key = i
                        break

            trip.append(list_of_keys[key])
            totalWeightOfTheTrip += list_of_values[key]
            list_of_keys.pop(key)
            list_of_values.pop(key)

            if len(list_of_keys) == 0 or totalWeightOfTheTrip + min(list_of_values) > limit:
                loopVariable = False
